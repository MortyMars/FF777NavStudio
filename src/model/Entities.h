...
// ============================================================================
// Entities.h
// Modèle de données interne du générateur de procédures SID/STAR/Approach
// pour le FMC FlightFactor B777v2 (X-Plane 12).
//
// Traduction typée de la spec de référence NavDataFile.h (namespace
// ndbl::navdata::File), fournie par l'utilisateur. Les structures ci-dessous
// reprennent les mêmes champs, dans le même ordre — cet ordre est important :
// il correspond exactement à l'ordre des colonnes dans les fichiers texte
// finaux, où chaque ligne a la forme :
//
//     NOM_STRUCTURE index champ1 champ2 champ3 ...
//
// Exemple : POINT 12654 mIdent mLat mLong mMagVar mHoldCourse
//           mHoldDistInMeters mHoldTime mHoldSide
//
// Ce fichier ne reproduit PAS le format binaire packé de la spec d'origine
// (#pragma pack(4), char[N+1], uint16/sint32 bruts) : c'est un modèle
// éditable en mémoire, pas un blob binaire. Les identifiants inter-entités
// sont donc remplacés par des types forts (Id<Tag>) plutôt que des sint32
// nus, précisément pour rendre impossible à la compilation la confusion
// découverte en debug entre Procedure::mLegSequenceId et
// RunwayProcedureTransition::mLegSequenceId (cause du doublon de séquence
// de legs sur le ND).
//
// Pour la même raison, tous les champs flottants sont en double, PAS en
// float comme la spec d'origine (qui les voulait en float pour des
// contraintes de mémoire binaire packée qui ne nous concernent pas). Un
// float (~7 chiffres significatifs) ne suffit pas à représenter fidèlement
// une valeur convertie pieds->mètres à 6 décimales (ex. 1219.199961, 10
// chiffres significatifs) — bug constaté en pratique : un LegSequence::
// transitionInMeters stocké en float était réécrit 1219.199951 au lieu de
// 1219.199961. Corollaire pour tout code appelant : ne jamais suffixer un
// littéral flottant par 'f' lors de la construction d'une entité (2.3f
// perd déjà la précision AVANT même la conversion vers double) — toujours
...
// Identifiants forts
// ------------------------------------------------------------------------------------------------------------------------------------------
// Id<Tag> encapsule un qint32, comme les mXxxId de la spec d'origine, mais
// chaque famille d'entité a un type distinct : impossible d'assigner un
// LegSequenceId là où un ProcedureId est attendu, et impossible de recopier
// tel quel un Procedure::legSequenceId dans un
// RunwayProcedureTransition::legSequenceId sans un cast explicite — ce qui
// doit attirer l'œil en revue de code.
//
// IMPORTANT : Id::invalid() (-1) reste une valeur assignable. Le type ne
// garantit PAS à lui seul "jamais -1" pour RunwayProcedureTransition::
// legSequenceId (cf. mDataStructures.txt ligne ~330) — cette règle reste de
// la responsabilité du validateur (règle bloquante dédiée), au même titre
// que "legSequenceId != parentProcedure.legSequenceId".
// ============================================================================
...
// ------------------------------------------------------------------------------------------------------------------------------------------
// Dans la spec d'origine, ePointUsage n'a qu'un rôle documentaire. Ici il est
// rendu actif : Q_DECLARE_FLAGS permet au validateur de vérifier des règles
// sémantiques, par ex. le point posé comme seuil de piste dans une
// RunwayProcedureTransition doit porter le flag Runway ; un point IAF
// d'approche doit porter le flag Iaf, etc.
// ============================================================================
...
// Combinaisons documentées dans la spec d'origine (NavaidType_Approach,
// NavaidType_Transmit). Ce sont des combinaisons de flags, pas des valeurs
// isolées : elles ne peuvent donc pas être des enumerators — elles sont
// reconstituées ici comme constantes de type NavaidTypeFlags.
...
// Catégories numériques (0-3) et catégories à code lettre (IGS/LDA/SDF)
// cohabitent dans le même champ quint32 d'origine — stockées ici comme un
// enum unique dont les enumerators lettre prennent la valeur du code ASCII
// correspondant (ex. Igs = 'I' = 73), ce qui ne collisionne jamais avec les
// valeurs numériques 0-3.
...
// ------------------------------------------------------------------------------------------------------------------------------------------
// LegSequence::sequenceTypeRaw est un uint8 brut dont le sens dépend de LA
// STRUCTURE QUI RÉFÉRENCE la LegSequence (cf. commentaire détaillé sur
// LegSequence plus bas) — jamais d'une propriété stockée sur la LegSequence
// elle-même, une même séquence pouvant être référencée dans des contextes
// différents (ex. ancrage de seuil de piste partagé SID + STAR). Ce enum
// sert donc de paramètre contextuel côté validateur (ex. checkSequenceType
// (ProcedureFamily::Sid, leg.sequenceTypeRaw)), pas de champ de struct.
// ============================================================================
...
// ------------------------------------------------------------------------------------------------------------------------------------------
// CORRECTION (confirmée par un extrait du fichier mondial de production) :
// contrairement à une première hypothèse construite sur les seules données
// LFFA, SID et STAR n'ont PAS un espace d'ID partagé — ce sont deux
// compteurs INDÉPENDANTS. Preuve dans l'extrait mondial : [DEPARTURES]
// contient "PROCEDURE 0 111 1" et [ARRIVALS] contient séparément
// "PROCEDURE 0 56 89185" — le même id 0 existe légitimement dans les deux
// sections simultanément. L'absence de chevauchement observée sur LFFA
// (35945-35948 côté SID, 27597-27603 côté STAR) n'était qu'une coïncidence
// liée à l'état des deux compteurs à ce moment-là, pas une preuve de
// partage.
//
// Conséquence structurelle : Procedure, ProcedureTransition et
// RunwayProcedureTransition ne portent PLUS de champ kind. Chaque kind vit
// dans sa PROPRE EntityTable, avec son propre IdAllocator indépendant (cf.
// ProjectRepository) — c'est l'appartenance à la table qui porte kind,
// exactement le même principe que "un Id<Tag> n'a de sens que dans sa
// table" déjà vrai pour tous les autres identifiants de ce fichier. Ce enum
// ProcedureKind reste utile comme PARAMÈTRE (sélectionner la bonne table,
// le bon fichier _Xxx.txt), jamais comme champ stocké sur une entité.
// ============================================================================
...
struct Navaid {
    ...
    quint32  frequencyMHzTimes100 = 0;       // mFreq — confirmé : fréquence en MHz x100 (ex. 10895 -> 108.95 MHz).
                                              // La spec d'origine documente "Hz/10", incohérent avec les données
                                              // réelles LFFA ; l'unité ci-dessus est celle effectivement utilisée.
...
// ------------------------------------------------------------------------------------------------------------------------------------------
// RAPPEL (piège identifié en debug) : une RunwayProcedureTransition ne doit
// JAMAIS réutiliser le LegSequenceId de son Procedure parent. Elle doit
// référencer une LegSequence minimale dédiée (un seul Leg de type IF posé sur
// le point de seuil de piste).
//
// CORRECTION (confirmée sur données réelles LFFA) : sequenceTypeRaw N'A PAS
// de "famille" intrinsèque. Une même LegSequence-ancrage de seuil de piste
// est légitimement partagée entre un SID et un(des) STAR — ex. LFFA :
// LegSequence "RW07" référencée à la fois par une RunwayProcedureTransition
// de _RunProcTransSID.txt et une de _RunProcTransSTAR.txt. Le sens de
// sequenceTypeRaw dépend uniquement de LA STRUCTURE QUI RÉFÉRENCE cette
// LegSequence à un instant donné, jamais de la LegSequence elle-même :
...
//   - référencée par RunwayProcedureTransition (ancrage dédié) -> valeur
//     neutre (0 observé dans les données LFFA), sans signification
//     DepartureType_/ArrivalType_ propre — c'est un simple marqueur.
// Le validateur doit donc vérifier sequenceTypeRaw au niveau de chaque
// référence (avec le contexte de qui référence), jamais au niveau de
// LegSequence seule — d'où l'absence ici de tout champ "family" ou
...
struct Leg {
    ...
    double           navaidDistanceInMeters    = -1.0; // mNavaidDistanceInMeters (RHO) — 5.24. <0 -> non spécifié
...
// Liaison intermédiaire de SID/STAR. Une Procedure sans
// RunwayProcedureTransition correspondante n'apparaît pas correctement au
// MCDU (cf. cas des 3 STAR sur 7 identifié en debug) — règle de validateur.
// ============================================================================
struct Procedure {
    AirportId     airportId;       // mAirportId
...
// Le premier missed approach doit être ajouté à la séquence de legs de
// l'approche (5.10) — règle métier à porter par le validateur, pas par ce
// header : rien ici ne peut garantir l'ordre des Leg d'une LegSequence.
// ============================================================================
struct Approach {
    RunwayId      runwayId;                       // mRunwayId
...
// ============================================================================
// STRUCTURE PROCEDURETRANSITION — fichiers _ProcedureTransition_SID.txt /
// _ProcedureTransition_STAR.txt (enroute transition, 5.11)
// ============================================================================
...
// ============================================================================
// STRUCTURE RUNWAYPROCEDURETRANSITION — fichier
// _RunwayProcedureTransition_SID.txt / _RunwayProcedureTransition_STAR.txt
// Liaison SID/STAR depuis/vers une RUNWAY (5.11).
// ------------------------------------------------------------------------------------------------------------------------------------------
// DEUX RÈGLES BLOQUANTES issues du debug, à porter par le validateur (pas par
// le type, cf. remarque sur Id::invalid() en tête de fichier) :
//   1. legSequenceId != invalid()                          -> fait planter X-Plane
//   2. legSequenceId != parentProcedure.legSequenceId       -> doublon de legs sur le ND
// Cas SID sans transition de piste mais EOSID disponible : pas d'enregistrement
// sentinelle nécessaire, le writer omet simplement la ligne correspondante
// (fichier de sortie correctement nommé mais vide pour ce cas précis).
// ============================================================================
