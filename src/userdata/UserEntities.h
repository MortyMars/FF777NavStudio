...
// ============================================================================
// UserEntities.h
// Modèle de SAISIE — ce que l'utilisateur tape, avant toute résolution
// d'ident ou conversion d'unité. Distinct du modèle cible (model/Entities.h)
// : ici les références inter-entités sont du texte (QString), les altitudes
// sont en pieds là où l'utilisateur les pense en pieds, les listes de choix
// restent leur libellé humain plutôt que leur valeur numérique cible.
//
// Ce fichier est stocké tel quel en base (dernier état de saisie, retrouvé
// d'une session à l'autre) — la conversion vers model::ProjectRepository se
// fait à la demande, ligne par ligne, par le futur moteur de génération.
// Une conversion peut échouer (ident non encore résolvable) sans que la
// saisie elle-même soit invalide : ce header ne porte aucune règle de
// validité, seulement la forme des données saisies.
//
// SCHÉMAS DE RÉSOLUTION — quatre familles, redondantes à travers toutes les
// structures ci-dessous ; chaque champ texte référençant une autre entité
// est annoté avec l'un de ces quatre schémas :
//
...
//
// Le futur registre de résolution n'a besoin que de CES QUATRE opérations
// composées, jamais d'une résolution ad hoc par structure.
// ============================================================================
...
struct UserLegSequence {
    QString ident;                 // [direct, identité de CETTE table] 6 car. max, sans les guillemets (ajoutés au Writer)
